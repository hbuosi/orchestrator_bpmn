...
function labelBounds(
  ...
  if (straight) {
    // Place label below the flow line so it never sits inside the shape height range.
...
export async function generateBpmnXml(process: BpmnProcess): Promise<string> {
  ...
  for (const [id, b] of ctx.shapes) {
    ...
    // For gateways: add explicit BPMNLabel above the diamond so the question text
    // is readable instead of being crammed inside the 50×50 diamond bounding box.
    let labelXml = '';
    if (isGw) {
